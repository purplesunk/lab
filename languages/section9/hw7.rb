...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    def inbetween(v, end1, end2)
      ...
      e1 = end1 - epsilon <= v and v <= end2 + epsilon
      e2 = end2 - epsilon <= v and v <= end1 + epsilon
      e1 or e2
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2) # Segments on vertical line
      if @y1 < seg.y1
        a = self
        b = seg
      else
...
class Let < GeometryExpression
  ...
  def eval_prog env
    pr = [@s, @e1.eval_prog(env)]
    @e2.eval_prog(Array.new(env).unshift(pr))
  end
  def preprocess_prog
